...
# Build next generation of NN
    def BuildNextGeneration(self):
        ...
        #for i in range(len(genotypes)):
            #genotypes[i].parameters.sort() # может замедлять обучение!
        # Apply Selection
        intermediatePopulation = self.RemainderStochasticSampling(genotypes)
        # Apply Recombination
        newPopulation = self.RandomRecombination(intermediatePopulation, self.PopulationCount)
        # Apply Mutation
        mutPopulation = self.MutateAllButBestTwo(newPopulation) # newPopulation - список генетипов
        # set weights to NN
